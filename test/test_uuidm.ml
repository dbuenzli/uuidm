...
let test_id ?__POS__:pos version ?time_ms u us =
  ...
  if Uuidm.equal u Uuidm.nil then Test.int ~__POS__ variant 0x0 else
  if Uuidm.equal u Uuidm.max then Test.int ~__POS__ variant 0xF else
  Test.holds ~__POS__ (8 <= variant && variant <= 0xB);
  Test.int ~__POS__ (Uuidm.version u) version;
  Test.option ~__POS__ ~some:Test.Eq.int64 (Uuidm.time_ms u) time_ms;
  ()

let test_constants () =
  Test.test "Uuidm UUID constants" @@ fun () ->
  test_id ~__POS__ 0 Uuidm.nil     "00000000-0000-0000-0000-000000000000";
  test_id ~__POS__ 0xF Uuidm.max     "ffffffff-ffff-ffff-ffff-ffffffffffff";
  test_id ~__POS__ 1 Uuidm.ns_dns  "6ba7b810-9dad-11d1-80b4-00c04fd430c8";
  test_id ~__POS__ 1 Uuidm.ns_url  "6ba7b811-9dad-11d1-80b4-00c04fd430c8";
  test_id ~__POS__ 1 Uuidm.ns_oid  "6ba7b812-9dad-11d1-80b4-00c04fd430c8";
  test_id ~__POS__ 1 Uuidm.ns_X500 "6ba7b814-9dad-11d1-80b4-00c04fd430c8";
  ()
...
let test_gen () =
  ...
  test_id ~__POS__ 7 ~time_ms:0x1020_3040_5060L
  ...
  test_id ~__POS__ 7 ~time_ms:0x017F22E279B0L
    (Uuidm.v7
