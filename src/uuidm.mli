...
(** The type for UUID versions and generation parameters.
    ...
    {- [`V4] is random based with a private state seeded with
       [Random.State.make_self_init]. Use {!v4_gen} to specify your own seed.
       Use {!v4} to specify your own randomness.

       {b Warning.} Sequences of UUIDs generated using {!Random} are random
       but {e predictable} by an observer. If that is an issue use {!v4}
       with random bytes generated by a suitable CSPRNG.}} *)
...
val v4 : bytes -> t
(** [v4 b] is a random based V4 UUID that uses the first 16 bytes of
...
(** [v4 seed] is a function that generates random version 4 UUIDs with
    the given [seed]. {b Warning.} The sequence is random but
    predictable see {!version}. *)
